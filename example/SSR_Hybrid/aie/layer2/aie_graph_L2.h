...
class simpleGraph_k1_B6_L2: public graph {
  private:
    kernel mm[L2_NUM_KERNEL];

  public:
    port<input> in0[L2_NUM_KERNEL],in1[L2_NUM_KERNEL];
    port<output> out;
    
    simpleGraph_k1_B6_L2() {
      
      for (int i=0; i<L2_NUM_KERNEL;i++){
        if(i==0){
          mm[i] = kernel::create(mm1_kernel0_L2);
          source(mm[i]) = "mm1_kernel0_L2.cc";
        }
        else if(i==L2_NUM_KERNEL-1){
          mm[i] = kernel::create(mm1_kernel2_L2);
          source(mm[i]) = "mm1_kernel2_L2.cc";
        }
        
        else{
          mm[i] = kernel::create(mm1_kernel1_L2);
          source(mm[i]) = "mm1_kernel1_L2.cc";
        }
      ...
      for (int i=0; i<L2_NUM_KERNEL;i++){
        connect<window<L2_h1*L2_w1>>(in0[i], mm[i].in[0]);
        connect<window<L2_w1*L2_w2>>(in1[i], mm[i].in[1]);
      }
